...
public class ElpisParser implements Serializable {
    ...
    public ElpisParser(String entryPoint, String fileName, ElpisLexer lexer) {
        ...
        ifExp.nextTable.get(sym.terminalNames[sym.IDENTIFIER]).c.addRule(sym.terminalNames[sym.START], StackActions.POP, new Action() {
            @Override
            public Object act(Symbol arg) {
                Object op1 = astTree.tempMem.remove("$operand1");
                if (op1 != null) {
    ...
    void iterate(Node node, int index, Tuple<StackActions, Action, Node, Node> prev) {
        ...
        for(Map.Entry<String, Tuple<StackActions, Action, Node, Node>> next : node.nextTable.entrySet()) {
            ...
            if (next.getKey().equals("")) {
            ...
            } else if (node.name.equals("root") && tokens[index].value.equals("EMPTY") ||
                    (tokens[index].sym == sym.WORD && next.getKey().equals(tokens[index].value)) ||
                ...
                System.out.println("matched value : " + tokens[index].value + " , id : " + next.getKey());

                if (!(node.name.equals("root") && tokens[index].value.equals("EMPTY"))) {
                ...
                System.out.println("did not match value : " + tokens[index].value + " , id : " + next.getKey());
            }
        }
    }
    public void parse() {
